...
from level_generation.GenerationUtils import create_walled_room, create_hall, create_room, place_entities, place_stairs
from map_objects.Shapes import BSPRoom


class BSPTreeAlgorithm:
    def __init__(self):
        self.game_map = None
        self.room = None
        self._leafs = []
    ...
    def generate_level(self, game_map, dungeon_level, max_rooms, room_min_size, room_max_size, map_width, map_height,
        ...
        # BSP Algorithm
        self._leafs = self.iterate_bsp(entities, item_table, mob_table, map_width, map_height, 0, 0)
...
class MasterBSPTreeAlgorithm(BSPTreeAlgorithm):
    def generate_level(self, game_map, dungeon_level, max_rooms, room_min_size, room_max_size, map_width, map_height,
                       player, entities, item_table, mob_table):
        super(MasterBSPTreeAlgorithm, self).generate_level(game_map, dungeon_level, max_rooms, room_min_size,
                                                           room_max_size, map_width, map_height, player, entities,
                                                           item_table, mob_table
                                                           )
        """
        TODO: Modify BSPTreeAlgorithm such that it can take the coordinates of a room or just its coordinates and 
              generate leafs from that.
        Note: MasterBSPTreeAlgorithm is meant to be a top-level encapsulation for usage in the main game_map generation        
        """
