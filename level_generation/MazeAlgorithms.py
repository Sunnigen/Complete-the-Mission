...
class UserInterface:
    def __init__(self):
        ...
        libtcod.console_init_root(SCREEN_WIDTH, SCREEN_HEIGHT, 'Roguelike Dungeon Comparison',
        ...
        for y in range(MAP_HEIGHT):
            for x in range(MAP_WIDTH):
                if self.map.level[x][y] == 1:
                    libtcod.console_put_char_ex(self.con, x, y, '#', self.color_light_wall_fore,
                                                self.color_light_wall_back)
                else:
                    libtcod.console_put_char_ex(self.con, x, y, '.', self.color_light_ground_fore,
...
class RoomAddition:
    ...
    def addRoom(self, roomX, roomY, room):
        roomWidth, roomHeight = self.getRoomDimensions(room)
        roomX, roomY = int(roomX), int(roomY)
        for x in range(roomWidth):
            for y in range(roomHeight):
                if room[x][y] == 0:
