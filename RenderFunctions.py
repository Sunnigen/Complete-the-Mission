...
def obtain_viewport_dimensions(game_map, viewport_width, viewport_height):
    ...
    if player_x - viewport_width < 0:
        view_x_start = 0
        view_x_end = 2 * viewport_width + 1
    elif player_x + viewport_width + 1 > game_map.width:
        view_x_start = game_map.width - (2 * viewport_width) - 1
        view_x_end = game_map.width
    else:
        view_x_start = player_x - viewport_width
        view_x_end = player_x + viewport_width + 1

    if player_y - viewport_height < 0:
        view_y_start = 0
        view_y_end = 2 * viewport_height + 1
    elif player_y + viewport_height + 1 > game_map.height:
        view_y_start = game_map.height - (2 * viewport_height) - 1
        ...
        for y in range(view_y_start, view_y_end):
            for x in range(view_x_start, view_x_end):
                ...
                if visible:
                    ...
                    tile_char = game_map.tile_set[str(tile)].get('char')
                    # print('\ntile:', tile)
                    # print('tile_char:', tile_char)
                    # print('tile_color:', tile_color)
...
def render_all(con, panel, top_panel, entities, player, game_map, fov_map, enemy_fov_map, fov_recompute, message_log, screen_width, screen_height,
    ...
    viewport_width_start = -screen_width // 4
    viewport_height_start = -screen_height // 4
